  <meta name="viewport" content="width=device-width, initial-scale=1">
  <link rel="stylesheet" href="{{asset("/bower_components/admin-lte/plugins/fontawesome-free/css/all.min.css")}}">
  <link rel="stylesheet" href="{{asset("https://code.ionicframework.com/ionicons/2.0.1/css/ionicons.min.css")}}">
  <link rel="stylesheet" href="{{asset("/bower_components/admin-lte/plugins/datatables-bs4/css/dataTables.bootstrap4.css")}}">
  <link rel="stylesheet" href="{{asset("/bower_components/admin-lte/dist/css/adminlte.min.css")}}">
<body class="hold-transition sidebar-mini">
@include('application.includes.header')
@include('application.includes.sidebar')
<div class="content-wrapper">
@include('flash-message')
    <!-- Content Header (Page header) -->
    <div class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-6">
            <h3 class="m-0 text-dark">Dashboard</h3>
              <p>
              <?php
              echo $dzo = App\tbl_agency::where('id', Auth::user()->organization)->value('agency');
              echo ", ";
              echo $dzo = App\dzongkhag::where('dzongkhag_id', Auth::user()->dzongkhag_id)->value('dzongkhag_name');
              ?>
             </p>            
          </div><!-- /.col -->
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
              <li class="breadcrumb-item"><a href="#">Home</a></li>
              <li class="breadcrumb-item active">Commission for Religious Organizations</li>
            </ol>
          </div><!-- /.col -->
        </div><!-- /.row -->
      </div><!-- /.container-fluid -->
    </div>
    <!-- /.content-header -->
    <!-- Main content -->
<section class="content">
<div class="container-fluid">
<div class="row">
          <div class="col-12 col-sm-6 col-md-3">
            <div class="info-box" style="background-image: linear-gradient(to bottom right,  #fa9761, #eb9902);">
              <span class="info-box-icon bg-warning elevation-1"><i class="ion ion-clipboard"></i></span>
              <div class="info-box-content">
                <span class="info-box-text">RO Registrations</span>
                <span class="info-box-number">
                  <a href="{{route('application.home')}}" title="View RO Information">
                  <span class="btn btn-warning btn-sm" style="background-color: #f4b04a;">
                  <i class="fa fa fa-square" style="color: gray;"></i>
                  <b><?php echo $c = count(App\tbl_ro_detail::where('approve', '0')->get()); ?></b>&nbsp;&nbsp;Pending<br>&nbsp;&nbsp;&nbsp;
                  <i class="fa fa fa-check-square" style="color: gray;"></i>
                  <b><?php echo $c = count(App\tbl_ro_detail::where('approve', '1')->get()); ?></b>&nbsp;&nbsp;Registered
                  </span></a>
                </span>
              </div>
            </div>
          </div>
          <!-- /.col -->
          <div class="col-12 col-sm-6 col-md-3">
            <div class="info-box" style="background-image: linear-gradient(to bottom right,  #fa9761, #eb9902);">
              <span class="info-box-icon bg-warning elevation-1"><ion-icon name="create-outline"></ion-icon></span>
              <div class="info-box-content">
                <span class="info-box-text">Information Updates</span>
                <span class="info-box-number">
                  <a href="{{route('infoupdate_home')}}" title="View RO Information">
                  <span class="btn btn-warning btn-sm" style="background-color: #f4b04a;">
                  <i class="fa fa fa-square" style="color: gray;"></i>
                  <b><?php echo $c = count(App\tbl_info_update::where('approve', '0')->get()); ?></b>&nbsp;&nbsp;Pending<br>&nbsp;&nbsp;
                  <i class="fa fa fa-check-square" style="color: gray;"></i>
                  <b><?php echo $c = count(App\tbl_info_update::where('approve', '1')->get()); ?></b>&nbsp;&nbsp;Approved
                  </span></a>
                </span>
              </div>
            </div>
          </div>
          <!-- /.col -->
          <div class="col-12 col-sm-6 col-md-3">
            <div class="info-box" style="background-image: linear-gradient(to bottom right,  #fa9761, #eb9902);">
              <span class="info-box-icon bg-warning elevation-1"><i class="ion ion-card"></i></span>
              <div class="info-box-content">
                <span class="info-box-text">Visa Applications</span>
                <span class="info-box-number">
                  <a href="{{route('visa_application')}}" title="View Visa Applications">
                  <span class="btn btn-warning btn-sm" style="background-color: #f4b04a;">
                  <i class="fa fa fa-square" style="color: gray;"></i>
                  <b><?php echo $c = count(App\tbl_visa_application::where('approve', '0')->get()); ?></b>&nbsp;&nbsp;Pending<br>&nbsp;&nbsp;
                  <i class="fa fa fa-check-square" style="color: gray;"></i>
                  <b><?php echo $c = count(App\tbl_visa_application::where('approve', '1')->get()); ?></b>&nbsp;&nbsp;Approved
                  </span></a>
                </span>
              </div>
            </div>
          </div>
          <!-- /.col -->
          <div class="col-12 col-sm-6 col-md-3">
            <div class="info-box" style="background-image: linear-gradient(to bottom right,  #fa9761, #eb9902);">
              <span class="info-box-icon bg-warning elevation-1"><ion-icon name="information-circle-outline"></ion-icon></span>
              <div class="info-box-content">
                <span class="info-box-text">Event Informations</span>
                <span class="info-box-number">
                  <a href="{{route('eventinfo_view')}}" title="View Event Details">
                  <span class="btn btn-warning btn-sm" style="background-color: #f4b04a;">
                  <i class="fa fa fa-check-square" style="color: gray;"></i>
                  <b><?php echo $c = count(App\tbl_event_information::all()); ?></b>&nbsp;&nbsp;Events Submitted<br>&nbsp;&nbsp;
                  </span></a>
                </span>
              </div>
            </div>
          </div>
          <!-- /.col -->
          <div class="col-12 col-sm-6 col-md-3">
            <div class="info-box mb-3" style="background-image: linear-gradient(to bottom right,  #fa9761, #eb9902);">
              <span class="info-box-icon bg-warning elevation-1"><ion-icon name="cash-outline"></ion-icon></span>
              <div class="info-box-content">
                <span class="info-box-text">Annual Financial Reports</span>
                  <span class="info-box-number">
                  <a href="{{route('financial_all')}}" title="View Financial Submissions">
                  <span class="btn btn-warning btn-sm" style="background-color: #f4b04a;">
                  <i class="fa fa fa-square" style="color: gray;"></i>
                  <b><?php echo $c = count(App\tbl_financial_information::where('approve', '0')->get()); ?></b>&nbsp;&nbsp;Pending<br>&nbsp;&nbsp;
                  <i class="fa fa fa-check-square" style="color: gray;"></i>
                  <b><?php echo $c = count(App\tbl_financial_information::where('approve', '1')->get()); ?></b>&nbsp;&nbsp;Approved
                  </span></a>
                </span>
              </div>
            </div>
          </div>
          <!-- /.col -->
          <div class="col-12 col-sm-6 col-md-3">
            <div class="info-box" style="background-image: linear-gradient(to bottom right,  #fa9761, #eb9902);">
              <span class="info-box-icon bg-warning elevation-1"><ion-icon name="mail-outline"></ion-icon></span>
              <div class="info-box-content">
                <span class="info-box-text">Suggestions/Feedback</span>
                <span class="info-box-number">
                  <a href="{{route('compliant_view')}}" title="View Suggestions/Feedback">
                  <span class="btn btn-warning btn-sm" style="background-color: #f4b04a;">
                  <i class="fa fa fa-check-square" style="color: gray;"></i>
                  <b><?php echo $c = count(App\tbl_compliant::all()); ?></b>&nbsp;&nbsp;Feedbacks<br>&nbsp;&nbsp;
                  </span></a>
                </span>
              </div>
            </div>
          </div>
          <!-- /.col -->
          <div class="col-12 col-sm-6 col-md-3">
            <div class="info-box" style="background-image: linear-gradient(to bottom right,  #fa9761, #eb9902);">
              <span class="info-box-icon bg-warning elevation-1"><ion-icon name="location-outline"></ion-icon></span>
              <div class="info-box-content">
                <span class="info-box-text">Religious Sites</span>
                <span class="info-box-number">
                  <a href="{{route('siteinfo_view')}}" title="View Religious Site Details">
                  <span class="btn btn-warning btn-sm" style="background-color: #f4b04a;">
                  <i class="fa fa fa-check-square" style="color: gray;"></i>
                  <b><?php echo $c = count(App\tbl_site_information::all()); ?></b>&nbsp;&nbsp;Sites<br>&nbsp;&nbsp;
                  </span></a>
                </span>
              </div>
            </div>
          </div>
          <!-- /.col -->
          <div class="col-12 col-sm-6 col-md-3">
            <div class="info-box" style="background-image: linear-gradient(to bottom right,  #fa9761, #eb9902);">
              <span class="info-box-icon bg-warning elevation-1"><ion-icon name="people-circle-outline"></ion-icon></span>
              <div class="info-box-content">
                <span class="info-box-text">Registered Members</span>
                <span class="info-box-number">
                  <a href="{{route('member_register')}}" title="View Religious Members Details">
                  <span class="btn btn-warning btn-sm" style="background-color: #f4b04a;">
                  <i class="fa fa fa-check-square" style="color: gray;"></i>
                  <b><?php echo $c = count(App\tbl_newmember::all()); ?></b>&nbsp;&nbsp;Members<br>&nbsp;&nbsp;
                  </span></a>
                </span>
              </div>
            </div>
          </div>

</div>            


<!-- Main row -->
<div class="row">
<section class="col-lg-12 connectedSortable">
<!-- Registration -->
<div class="card direct-chat direct-chat-primary">
<div class="card-header">
                <div>
                  <div class="btn-group w-100 mb-2">
                    <a class="btn btn-warning active" href="{{route('application.home')}}" data-filter="1">Registration Applications</a>
                    <a class="btn btn-warning" href="{{route('infoupdate_home')}}" data-filter="2">Information Updates</a>
                    <a class="btn btn-warning" href="{{route('newinfo_cro_home')}}" data-filter="5">New Information</a>
                    <a class="btn btn-warning" href="{{route('financial_all')}}" data-filter="3">Financial Reports</a>
                    <a class="btn btn-warning" href="{{route('visa_application')}}" data-filter="4">Visa Applications</a>
                  </div>
                </div>                       
            <div class="box-header">
            <h6 class="box-title">All Registration Applications</h6>
            </div>
            <!-- /.box-header -->
  <div class="content-header">
  <div class="container-fluid">
            <table id="example1" class="table table-bordered table-striped">
            <thead>
              <tr style="text-align: left;">  
                  <th>No</th>
                  <th>RO Name</th>
                  <th style="width: 5cm;">Address</th>
                  <th>Chairman</th>
                  <th>Contact</th>
                  <th>Dated</th>
                  <th>Action</th>
              </tr>
            </thead>
            <tbody>
              <?php $id = 1;?>
              @foreach($ro as $dc)
              <tr>
                  <td>{{$id++}}</td>
                  <td>{{ $dc->name }}</td>
                  <td style="width: 5cm;">{{ $dc->location }}</td>
                  <td><?php echo $c = App\tbl_chairman_detail::where('ro_id', $dc->ro_id)->value('name'); ?></td>
                  <td>{{ $dc->phone }}</td>
                  <td><?php echo date('d/m/Y', strtotime($dc->created_at));?></td>
                  <td>
                  @if($dc->approve == '0')
                    <a href="{{ route('edit_ro', $dc->ro_id) }}" title="Assessment/Review">
                    <span class="btn btn-success btn-sm"><i class="fa fa-fw fa-paste"></i>Review</span>@endif
                  @if($dc->approve == '1')<span class="btn btn-warning btn-sm">Approved</span>@endif
                  @if($dc->approve == '2')<span class="btn btn-danger btn-sm">Rejected</span>@endif  
                  </td>
              </tr> 
              @endforeach
            </tbody>
            </table>   
  </div>
  </div>
</div>
</div>
          
</section>  
</div>

</div><!-- /.container-fluid -->
</section>
<!-- /.content -->
</div>
<!-- /.content-wrapper -->
@include('application.includes.footer')
</body>
<script src="{{asset("/bower_components/admin-lte/plugins/datatables/jquery.dataTables.min.js")}}"></script>
<script src="{{asset("/bower_components/admin-lte/plugins/datatables/dataTables.bootstrap.min.js")}}"></script>
<link rel="stylesheet" href="{{asset("/bower_components/admin-lte/plugins/datatables/dataTables.bootstrap.css")}}">
<script src="{{asset("/bower_components/admin-lte/plugins/datatables-bs4/js/dataTables.bootstrap4.min.js")}}"></script>
<script>
  $(function () {
    $("#example1").DataTable();
    ({
     
      "paging": true,
      "lengthChange": true,
      "searching": true,
      "ordering": true,
      "info": true,
      "autoWidth": false,
      "responsive": true,
    });
  }); 
</script>


 
