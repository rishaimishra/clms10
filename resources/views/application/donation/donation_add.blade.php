...
<p>Donation Collection Team.&nbsp;&nbsp;&nbsp;(Maximum of 6 Members)</p>
<div class="row">
<form action="{{ route('cid_search_donation', $donation_id) }}" method="POST" enctype="multipart/form-data">
{{ csrf_field() }}
          <div class="col-md-12">
          <div class="input-group input-group-sm mb-0">
            <label for="cid">CID</label>&nbsp;&nbsp;&nbsp;&nbsp;
            <input class="form-control form-control-sm" name='cidno' placeholder="Enter CID number">
            <div class="input-group-append">
            <button type="submit" class="btn btn-danger">FETCH DETAILS</button>
            </div>
          </div>
          </div> 
...
<input id="name" type="hidden" class="form-control" name="name" value="<?php echo $cDetail1[0]['firstName']." ".$cDetail1[0]['lastName'];?>">
    ...
    <table id="example1" class="table table-bordered table-striped">
            ...
            <tbody>
              ...
              <tr>
                  ...
                  <td>{{ $dc->cid }}</td>
                  <td>{{ $dc->name }}</td>
                  <td><?php if (is_numeric($dc->dzongkhag)) { echo "<?php echo $dzo = App\dzongkhag::where('dzongkhag_id',$dc->dzongkhag)->value('dzongkhag_name');?>"; } else { echo "$dc->dzongkhag"; } ?>
                  </td>
                  <td><?php if (is_numeric($dc->gewog)) { echo "<?php echo $dzo = App\gewog::where('gewog_id',$dc->gewog)->value('gewog_name');?>"; } else { echo "$dc->gewog"; } ?>
                  </td>
                  <td>
                    <a href="{{ route('destroy_collector', array($dc->id, $dc->donation_id)) }}" onclick="return confirm('Are you sure, you want to Delete ?')">
...
<form class="search-form" role="form" method="POST" action="{{ route('store_donationcollectors') }}">  
{{ csrf_field() }} 
<input type="hidden" name="donation_id" value="{{$donation_id}}">
<div class="row">
<div class="col-sm-2"></div>
...
@if( ! $donss )
   <div class="form-row">
     <label class="col-sm-4 col-form-label"  for="purpose">Purpose of Collection:<font color="red">*</font></label>
     <textarea type="text" id="purpose" class="form-control col-sm-8" name="purpose" rows="3" required/></textarea>
   </div>
@else
   <div class="form-row">
     <label class="col-sm-4 col-form-label"  for="purpose">Purpose of Collection:</label>
     <?php echo $donss = App\tbl_donation::where('donation_id', $donation_id)->value('purpose'); ?>
   </div><br>
@endif
<button type="submit" class="btn btn-primary btn-sm">SUBMIT</button>


<button class="btn btn-primary btn-bg"><h3><a href="{{ route('donation_dzo') }}" style="color: white;" title="Finish and return to dashboard"><i class="fa fa-fw fa-home"></i>&nbsp;གདོང་ཤོག</a></h3></button>
