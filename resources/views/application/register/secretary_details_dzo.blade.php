...
@if($token_no != '' && $sroid == '')
              <div class="card-body">
                <div>
                  <div class="btn-group w-100 mb-2">
                    ...
                    <a class="btn btn-info active" href="{{route('secretary_details_dzo',$ro_id)}}" data-filter="4"><h3>དྲུང་ཆེན་གྱི་ཁ་གསལ། </h3></a>
            ...
            <div class="row">
                <div class="col-md-4">
                <div class="card-body">
                  <form action="{{ route('cid_search_sdreg_dzo',$ro_id) }}" method="POST" enctype="multipart/form-data">
                        <div class="input-group input-group-sm mb-0">
                          <label for="cid"><h4>ངོ་སྤྲོད་ལག་ཁྱེར་ཨང་</h4></label>&nbsp;&nbsp;&nbsp;&nbsp;
                          <input class="form-control form-control-sm" placeholder="Enter CID number" required/>
                          <div class="input-group-append">
                            <button type="submit" class="btn btn-danger"><h3>འཚོལ།</h3></button>
                          </div>
                        </div>
                  </form>
  <form role="form" method="POST" enctype="multipart/form-data" action="{{ route('secretary_details_store_dzo') }}">
  <input type="hidden" name="_token" id="token" value="{{ csrf_token() }}">
  <input type="hidden" name="dets" value="{{$dets}}">
  {{ csrf_field() }}
                  <div class="form-group input-group-sm">
                    <label for="name"><h3>མིང་</h3></label><font color="red">*</font>
                    <input type="text" class="form-control" name="name" placeholder="Enter Organization Name" required="required"/>
                  </div>
                  <div class="form-group input-group-sm">
...
<div class="form-group input-group-sm">
<label for="chairman"><h3>དྲུང་ཆེན་གྱི་ཡིག་ཆ།</h3></label><font color="red">*</font>
...
@foreach($data as $app)
  <form role="form" method="POST" enctype="multipart/form-data" action="{{ route('secretary_details_store_edit_dzo') }}">
  <input type="hidden" name="_token" id="token" value="{{ csrf_token() }}">
...
<div class="form-group input-group-sm">
<label for="chairman"><h3>དྲུང་ཆེན་གྱི་ཡིག་ཆ།</h3></label>
<input type="file" id="exampleInputFile2"  class="form-control" name="sd[]" multiple="">
</div>
@foreach($droid as $dd)
@if($dd->filecat == 'sd')
<h4>དྲུང་ཆེན་གྱི་ཡིག་ཆ།</h4>&nbsp;&nbsp;&nbsp;{{ $dd->file_name }}
